...
jQuery(document).ready(function ($) {
  ...
  */
  ...
  window.onload = function () {
    ...
    if (hash != "") {
      ...
      $(artist).find('.copy').addClass('active'); // alert(artist);
      ...
      var $blocks = $(artist).nextAll('div'); // alert(top);
      ...
      $blocks.each(function (i, j) {
        if ($(artist).offset().top != top) {
          placeAfter($(artist).prev('div')); // alert('!=');

          return false;
        } else if (i + 1 == $blocks.length) {
          placeAfter($(artist)); // alert('+1');
  ...
  $('.wrapblock').on('click', function () {
    if ($(this).find('.artist-title').hasClass('active')) {
      ...
      $('#dude').css('display', 'none'); //console.log('Closing pop up...');
    } else {
      // open pop up
      //console.log('Opening pop up...');
      $('h2.artist-title').removeClass('active');
      $(this).find('.artist-title').addClass('active');
      $(".art-contents").empty(); //$(this).next('.showfull').addClass('active');
      ...
      $blocks.each(function (i, j) {
        if ($(this).offset().top != top) {
          placeAfter($(this).prev('.wrapblock')); // alert('!=');

          return false;
        } else if (i + 1 == $blocks.length) {
          placeAfter($(this)); // alert('1');
  ...
  */
  ...
  $('#filters select').each(function () {
    ...
    var $listItems = $list.children('li'); // $styledSelect.click(function(e) {
    //     alert($styledSelect);
    //     e.stopPropagation();
    ...
    $('.days .select .select-options li').click(function (e) {
      e.stopPropagation();
      $('.select-styled .dayz').text($(this).text()).removeClass('active'); // $this.val($(this).attr('rel'));

      $('.days .select-options').hide(); //console.log($this.val());
      // alert($this.val());

      var rel = $(this).attr('rel');
      $(".js-day").css("display", "none"); // var val = $(this).val();
      //alert(rel);
  ...
  *
  ...
  $('select').change(function () {
    ...
    console.log($this.find(':selected')); // convert object into array
  ...
  *
  ...
  $('.flexslider').imagesLoaded(function () {
    $('.flexslider').flexslider(_defineProperty({
      animation: "fade",
      smoothHeight: true,
